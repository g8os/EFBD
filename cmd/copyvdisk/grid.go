...
func getConnectionsFromGrid(logger log.Logger, input *userInput) (connA, connB redis.Conn, err error) {
	cfgA, err := storagecluster.NewClusterConfig(input.Source.URL, input.Source.VdiskID, logger)
	if err != nil {
		return
	}
	...
	cfgB, err := storagecluster.NewClusterConfig(input.Target.URL, input.Target.VdiskID, logger)
	if err != nil {
		return
	}
